-- main.lua - ReaScript controller with structured communication
...
function run_full_workflow(session_config)
    ...
    local workflow_steps = {
        ...
        {"change_params.lua", "Changing target parameter to " .. (session_config.parameter_value or "0.0")},
    ...
    for i, step in ipairs(workflow_steps) do
        ...
        if not success then
            create_beacon_file("ERROR", "Workflow failed at step " .. i .. ": " .. description, "main.lua")
            return false
        end
    end

    -- Workflow completed successfully
    create_beacon_file("COMPLETED", "Full workflow completed for session " .. (session_config.session_id or "unknown"), "main.lua", 1.0)
    return true
end
...
function run_discovery_workflow(session_config)
    ...
    if success then
        create_beacon_file("COMPLETED", "Parameter discovery completed", "main.lua", 1.0)
    else
        create_beacon_file("ERROR", "Parameter discovery failed", "main.lua")
    end
...
function run_single_script_workflow(script_name, session_config)
    ...
    if success then
        create_beacon_file("COMPLETED", "Script " .. script_name .. " completed", "main.lua", 1.0)
    else
        create_beacon_file("ERROR", "Script " .. script_name .. " failed", "main.lua")
    end
...
function main()
    ...
    if not session_config then
        create_beacon_file("ERROR", "Could not read configuration file", "main.lua")
        return
    end
    ...
    elseif workflow_mode == "single" then
        ...
        create_beacon_file("ERROR", "Unknown workflow mode: " .. workflow_mode, "main.lua")
        return
    end
