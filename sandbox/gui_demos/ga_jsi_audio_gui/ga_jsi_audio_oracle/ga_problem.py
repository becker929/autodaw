...
from .jsi_ga_integration import JSIFitnessEvaluator, GAPopulationRanker
...
class HumanJSIAudioOptimizationProblem(JSIAudioOptimizationProblem):
    """GA problem that uses human audio comparisons instead of automated oracle."""

    def __init__(
        self,
        reaper_project_path: Path,
        human_oracle,  # HumanAudioComparisonOracle instance
        session_name_prefix: str = "human_jsi_audio_ga",
        show_live_ranking: bool = True
    ):
        """Initialize human JSI audio optimization problem.

        Args:
            reaper_project_path: Path to REAPER project directory
            human_oracle: Human audio comparison oracle instance
            session_name_prefix: Prefix for session names
            show_live_ranking: Whether to show live JSI ranking updates
        """
        # Define problem dimensions (same as parent)
        n_var = 2  # octave, fine
        n_obj = 1  # Single objective: maximize JSI-derived fitness
        xl = np.array([-2.0, -1.0])  # Lower bounds
        xu = np.array([2.0, 1.0])    # Upper bounds

        # Initialize as Problem directly (skip parent __init__)
        Problem.__init__(self, n_var=n_var, n_obj=n_obj, xl=xl, xu=xu)
        ...
        # Use the provided human oracle
        self.oracle = human_oracle
        print(f"Using human oracle: {type(human_oracle).__name__}")

        # Initialize JSI population ranker directly (not the evaluator)
        self.population_ranker = GAPopulationRanker(
            oracle=self.oracle,
            console=None,  # Will create when needed
            show_live_ranking=show_live_ranking
        )
        
        # Create a dummy jsi_evaluator for compatibility with _log_generation_stats
        class DummyEvaluator:
            ...
            def get_ranking_info(self):
                return {
                    'comparison_count': self.problem.oracle.get_comparison_count(),
                    'generation_count': self.problem.generation_counter
                }
        
        self.jsi_evaluator = DummyEvaluator(self)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        """Evaluate population using human JSI ranking."""
        self.generation_counter += 1
        session_name = f"{self.session_name_prefix}_gen_{self.generation_counter:03d}"

        print(f"\n=== Human Evaluation Generation {self.generation_counter} ===")
        print(f"Population size: {len(x)}")
        print("You will be asked to compare audio samples to guide the evolution...")
        ...
        try:
            # Step 1: Render audio using REAPER
            print("Rendering audio samples...")
            audio_paths = self._render_population_audio(solutions, session_name)

            # Step 2: Use JSI + human oracle to rank population
            print("Starting human comparisons...")
            ranked_solutions, fitness_values, ranking_info = self.population_ranker.rank_population_with_audio(
                solutions, audio_paths, self.generation_counter
            )
            ...
            # Log generation statistics
            self._log_generation_stats(solutions, fitness_values)
            print(f"Human comparisons made this generation: {ranking_info.get('comparisons_made', 0)}")
            print(f"Total human comparisons: {self.oracle.get_comparison_count()}")

            # Cleanup old renders periodically
            if self.generation_counter % 3 == 0:
                self._cleanup_old_renders(keep_generations=2)

            # Convert to numpy array for pymoo (negate for minimization)
            out["F"] = np.array([-f for f in fitness_values]).reshape(-1, 1)

        except Exception as e:
            print(f"Error during human evaluation: {e}")
            import traceback
            traceback.print_exc()
    ...
    def get_best_solution_info(self, result) -> Optional[Dict[str, Any]]:
        """Get information about the best solution found."""
        ...
        try:
            ...
            octave, fine = best_x
            best_solution = Solution(octave=octave, fine=fine)

            return {
                'solution': f"octave={octave:.3f}, fine={fine:.3f}",
                'fitness': -best_f,  # Convert back from minimization
                'frequency_ratio': f"{octave:.3f} octaves + {fine:.3f} fine",
                'evaluations': self.evaluation_count,
                'human_comparisons': self.oracle.get_comparison_count()
            }

        except Exception as e:
            print(f"Error extracting best solution info: {e}")
            return None
