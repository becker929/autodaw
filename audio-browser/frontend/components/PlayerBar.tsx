 ...
 * element for the whole application. Moving from the list to a detail page to
 * the playlist re-renders the view area and leaves this bar and its audio
 * element mounted, which is what lets a sound keep playing across a
 * navigation.
 *
 ...
 *
 * On a narrow screen it stacks: the name and favourite on one line, the
 * read-out on the next, then the waveform and the transport side by side. The
 * name scrolls itself when it is too long to fit. See `Marquee`.
 */
    ...
    <div className="playerbar" data-testid="player-bar" data-hash={row.hash}>
      <div className="who">
        ...
        {/* One line, and it is clipped on a phone, so it runs in order of what
            ...
            refused, what is being skipped, then the queue position and the
            path count, which the list and the detail page also carry. */}
        <div className="sub mono" data-testid="player-readout">
          ...
          {player.index !== null
            ...
            : ""}
          {row.alias_count > 1 ? ` · ${row.alias_count} paths` : ""}
        </div>
      </div>
      ...
      {/* Star and discard, side by side, where the thumb already is. Listening
          to a sound and judging it is one gesture; the list should not have to
          be scrolled back to to say what the sound was. */}
      <div className="acts">
        ...
        <button
          ...
          onClick={() => void player.toggleFavorite(row)}
        >
          {row.favorite ? "★" : "☆"}
        </button>
        <button
